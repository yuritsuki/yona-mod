...
const
  (* Event arguments *)
  ARG_ATTACKER_STACK        = 1;
  ARG_DEFENDER_STACK        = 2;
  ARG_FINAL_DAMAGE_CONST    = 3;
  ARG_FINAL_DAMAGE          = 4;
  ARG_BASIC_DAMAGE          = 5;
  ARG_DAMAGE_BONUS          = 6;
  ARG_IS_DISTANT_ATTACK     = 7;
  ARG_DISTANCE              = 8;
  ARG_IS_THEORETICAL_ATTACK = 9;
...
begin
  Attacker := Era.x[ARG_ATTACKER_STACK];
  Defender := Era.x[ARG_DEFENDER_STACK];
  
  FinalDamage         := Era.x[ARG_FINAL_DAMAGE];
  BasicDamage         := Era.x[ARG_BASIC_DAMAGE];
  DamageBonus         := 0;
  IsDistantAttack     := Era.x[ARG_IS_DISTANT_ATTACK] = 1;
  Distance            := Era.x[ARG_DISTANCE];
  IsTheoreticalAttack := Era.x[ARG_IS_THEORETICAL_ATTACK] = 1;
  
  IngameDamageMultiplier := FinalDamage / (BasicDamage + Era.x[ARG_DAMAGE_BONUS]);
  DamageMultiplier       := IngameDamageMultiplier;
  DamagePercentBonus     := Era.x[ARG_DAMAGE_BONUS] * (100 / BasicDamage);
  ...
  if DebugStackToStackDmgOpt then begin
    ...
    (
      'DM = (',
      BasicDamage, ' + ', (DamagePercentBonus):1:1, '% (', Era.x[ARG_DAMAGE_BONUS],
  ...
  if Defender <> NO_STACK then begin
    ...
    Era.x[ARG_FINAL_DAMAGE] :=  Math.Max(1, TRUNC
    (
...
begin
  if not Game.CursorLocked or SpecialMode then begin
    ...
    MinDamage := Era.x[ARG_MIN_DAMAGE];
    MaxDamage := Era.x[ARG_MAX_DAMAGE];
...
begin
  KeyCode          := Era.x[ARG_KEYCODE];
  BlockDefReaction := LONGBOOL(Era.x[ARG_BLOCKDEFREACTION]);
  ...
  Era.x[ARG_KEYCODE]          :=  KeyCode;
  Era.x[ARG_BLOCKDEFREACTION] :=  integer(BlockDefReaction);
